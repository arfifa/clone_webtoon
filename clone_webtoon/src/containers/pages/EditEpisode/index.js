...
class EditEpisode extends Component {
  ...
  _editEpisode() {
    ...
    if (imageUri != null && releaseDate != "" && episode.length > 0 && imageToon.length > 0) {
      ...
      alert("Episode successfully edited!")
    } else {
  ...
  render() {
    const { episode, releaseDate, imageUri, imageToon, _isValid, _isEdit } = this.state
    const { navigation } = this.props
    if (_isEdit == true) {
      this.setState(navigation.getParam('dataEditEpisode'))
      this.setState({
        _isEdit: false
      })
    }
    return (
      <Container style={nbStyles.container}>
        <MainHeader
          ...
          onPressLeft={() => this.props.navigation.navigate('EditWebtoon', _isValid ? {
            dataEditEpisode: this.state
          } : {
              dataEditEpisode: undefined
            })}
          onPressRight={() => this._editEpisode()}
        />
        <Content>
          <CardItem style={nbStyles.cardItem}>
            <Body style={nbStyles.cardBody}>
              ...
              <Text style={nbStyles.episodeTitle}>Add Image Episode</Text>
              ...
              <Button full style={nbStyles.delButtonEpisode} onPress={() => this.props.navigation.navigate('EditWebtoon', {
                deleteEpisode: this.state.id
              })}>
                <Text>DElETE</Text>
              </Button>
            </Body>
