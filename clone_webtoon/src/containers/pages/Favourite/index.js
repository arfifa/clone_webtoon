...
class Favourite extends Component {
  ...
  _handleSearch(typeWord) {
    this.setState({ searchWord: typeWord })
  }

  render() {
    let filterComics = this.state.comicData.filter(
      (data) => (data.title.toLowerCase().indexOf(this.state.searchWord.toLowerCase()) !== -1)
    )
    return (
      <Container style={nbStyles.container}>
        <HeaderSearch comicSearch={(typeWord) => this._handleSearch(typeWord)} />
        <Content>
                ...
                <TouchableOpacity
                  ...
                  <View style={nbStyles.card}>
                    ...
                    <View style={nbStyles.listTextContainer}>
                      ...
                      <Text style={nbStyles.cardSubTitle}>{item.favoriteCount} Favourite</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              )}
