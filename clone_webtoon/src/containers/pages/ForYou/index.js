...
import { Banner, AllToon, FavouriteHscroll } from '../../organisms';
import { HeaderSearch } from '../../../components/molecules/Header';
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      profile: [
        {
          nama: 'Arfifa Rahman',
          profileImageUri: '../../../assets/image/DevPhoto/pasPhoto.png'
        }
      ]
    }
  }

  _handleAddFavourite(data) {
    const { comicData } = this.state
    this.state.favourite.push({ ...data })
    let changeFavouriteStatus = comicData.find(comicData => data.id === comicData.id)
    if (changeFavouriteStatus) {
      changeFavouriteStatus.favouriteStatus = !data.favouriteStatus
      this.setState([...comicData])
    }
  }

  _handleDeleteFavourite(comicId) {
    const { comicData, favourite } = this.state
    const filterData = favourite.filter(function (data) { return data.id != comicId })
    this.setState({ favourite: filterData })
    let changeFavouriteStatus = comicData.find(comicData => comicId === comicData.id)
    if (changeFavouriteStatus) {
      changeFavouriteStatus.favouriteStatus = false
      this.setState([...comicData])
    }
  }
  ...
  render() {
    return (
      <Container style={nbStyle.container}>
        <HeaderSearch />
        <Content>
          <Banner comicData={this.state.comicData} />
          <FavouriteHscroll favourite={this.state.favourite} />
          <AllToon
            comicData={this.state.comicData}
            addFavourite={(comicData) => this._handleAddFavourite(comicData)}
            deleteFavourite={(comicId) => this._handleDeleteFavourite(comicId)} />
        </Content>
      </Container>
    )
  }
...
const nbStyle = {
  container: { backgroundColor: theme.colors.lightBrown, flex: 1 },
}
